...
# Try importing OpenCV and MediaPipe
try:
    import cv2
    ...
    st.error(f"Error loading OpenCV or MediaPipe: {e}")
    st.info("Please make sure you have the correct dependencies installed. Check the requirements.txt file.")
    st.stop()
...
with st.expander("📝 How to Use", expanded=True):
    ...
    <div class="instructions">
        ...
        <ul>
            ...
            <li>If you're using this in the browser, make sure to allow camera access</li>
    ...
    if video_path is not None:
        try:
            ...
            if success:
                ...
                st.error("Failed to read from video source. Please check your camera permissions and try again.")
...
# Toggle camera state
if st.button("Start/Stop Camera", key="camera_toggle"):
    st.session_state.camera_running = not st.session_state.camera_running
    if st.session_state.camera_running:
        run_handwriting_app()

# Add a link back to portfolio
st.markdown("---")
st.markdown("[← Back to Portfolio](https://your-portfolio-url.com)")
...
Developed by Sai Rupa Jhade | <a href="https://github.com/Jsairupa/virtual-handwriting" target="_blank">GitHub Repository</a>
</div>
""", unsafe_allow_html=True)
